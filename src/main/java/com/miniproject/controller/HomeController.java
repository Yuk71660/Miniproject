...
import com.miniproject.dao.blog.BlogDAO;
...
public class HomeController {
	...
	public ResponseEntity<String> searchBlog(@RequestParam("keyWord") String keyWord) {
		...
		try {
			...
			System.out.println(objItems.size());

			for (Object item : objItems) {
				JSONObject jsonPost = (JSONObject) item;

				BlogPostDto post = new BlogPostDto(
						(String) jsonPost.get("link"), (String) jsonPost.get("postdate"),
						(String) jsonPost.get("description"), (String) jsonPost.get("title"),
						(String) jsonPost.get("bloggerlink"), (String) jsonPost.get("bloggername")
					);

				blogPosts.add(post);
			}

			bdao.savePosts(blogPosts);
