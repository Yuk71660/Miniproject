...
public class MemberServiceImpl implements MemberService {
   ...
   public boolean registerMember(MemberDTO newMember) throws Exception {
      ...
      // 1) newMember를 테이블에 insert하기 전에 수행해야 할 작업(취미를 하나의 문자열로 만들기, 도로명주소 + 상세 주소를 하나의 문자열로 합치기)을 완료하고, dao 단 호출
...
//      System.out.println(newMember.toString());
      ...
      if (dao.insertMember(newMember) == 1) {
         ...
         PointLogDTO pdto = PointLogDTO.builder()
               .pointwho(newMember.getUserId())
               .pointwhy("회원가입")
               .build();
